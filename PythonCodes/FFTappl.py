...
N = 1024                     # 2**10 
signalgr = gdisplay(x=0,y=0,width=500,height=250, title='Gaussian signal',xtitle='x',
                    ytitle='y',xmax=25.0,xmin=-25.0,ymax=1,ymin=0)
...
tipl    = gvbars(delta=0.01,color=color.magenta,display=transgr)   # Imag
invgr   = gdisplay(x=0,y=500,width=500,height=250, title='inverse TF',\
     xtitle='x',ytitle='y',xmax=25.0,xmin=-25.0,ymax=1,ymin=0)  # inverse
...
sigma    = 3.0                                        # width wave packet
foursig2 = 4.0*sigma*sigma;
def gaussian():
    psr = zeros((N+1),float)
    psi = zeros((N+1),float)
    len = 50.0                                         # x from -25 to 25
    # m = N;
    a = 0.0                         # initial position wave packet center
    dxx = 50.0/N                                         # increment in x
    xx = -0.5*len                          # left end in coordinate space
    for n in range(0,N):# loop over space coordinates to define V and Psi
        xma2 = (xx-a)*(xx-a)/foursig2          # a=0 :initial wave packet
        psr[n] = exp(-xma2)              # real part initial wavefunction
        psi[n] = 0.0                         # imaginary part initial w.f.                                         
        dtr[n,0] = psr[n];                      # Re part for data to ftt
        dtr[n,1] = psi[n];                       # Im part for data to fft
...
gaussian()                                       # call original function
