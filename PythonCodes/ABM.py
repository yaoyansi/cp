...
A = 0; B = 3.                               # Endpoints
t =[0]*500;     y =[0]*500;     yy=[0]*4    # Initialize
                        
def f(t, y):                    # Function returns RHS (force function)
    return  (t - y)/2.0

def rk4(t, yy, h1):             # Function for rk4 values
    for i in range(0, 3):
        t  = h1 * i
        k0 = h1 * f(t, y[i])
...
# Compute 3 additional starting values using rk
   ...
   for k in range(1, 4):    # values for t
      t[k] = a  +  k * h
   y[1]  = rk4(t[1], y, h)  # for 1st step
   y[2]  = rk4(t[2], y, h)  # for 2nd step
   y[3] = rk4(t[3], y, h)   # for 3rd step
   F1 = f(t[1], y[1])       # evaluate function
   F2 = f(t[2], y[2])       # at these steps
   F3 = f(t[3], y[3])
   h2 = h/24.
